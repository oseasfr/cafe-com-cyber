...
function ArticleContent({ article, theme }: { article: typeof articles[0]; theme: 'light' | 'dark' }) {
  ...
  // Texto específico onde deve ser inserido o PasswordGenerator
  const markerText = 'O Café com Cyber tem seu próprio gerador de senhas, que também valida a força das suas senhas atuais:';
  const shouldInsertPasswordGenerator = article.id === 'quando-foi-a-ultima-vez-que-voce-alterou-sua-senha' && article.content.includes(markerText);

  // Se deve inserir o PasswordGenerator, processamos o conteúdo
  let contentParts: { type: 'markdown' | 'component'; content?: string }[] = [];
  
  if (shouldInsertPasswordGenerator) {
    const markerIndex = article.content.indexOf(markerText);
    if (markerIndex !== -1) {
      // Encontra o início do parágrafo (última quebra de linha dupla antes do texto)
      const beforeMarker = article.content.substring(0, markerIndex);
      const paragraphStart = beforeMarker.lastIndexOf('\n\n');
      const actualStart = paragraphStart !== -1 ? paragraphStart + 2 : 0;
      
      // Encontra o final do parágrafo (próxima quebra de linha dupla após o texto)
      const afterMarker = markerIndex + markerText.length;
      const paragraphEnd = article.content.indexOf('\n\n', afterMarker);
      const actualEnd = paragraphEnd !== -1 ? paragraphEnd + 2 : article.content.length;
      
      // Divide o conteúdo em partes
      const beforeText = article.content.substring(0, actualEnd);
      const afterText = article.content.substring(actualEnd);
      
      contentParts = [
        { type: 'markdown', content: beforeText },
        { type: 'component' },
        { type: 'markdown', content: afterText }
      ];
    } else {
      contentParts = [{ type: 'markdown', content: article.content }];
    }
  } else {
    contentParts = [{ type: 'markdown', content: article.content }];
  }
  ...
  const markdownComponents = {
    h1: ({node, ...props}: any) => <h1 className={`text-3xl font-bold mt-8 mb-4 ${isLight ? 'text-gray-900' : 'text-foreground'}`} {...props} />,
    h2: ({node, ...props}: any) => <h2 className={`text-2xl font-bold mt-6 mb-3 ${isLight ? 'text-gray-800' : 'text-foreground'}`} {...props} />,
    h3: ({node, ...props}: any) => <h3 className={`text-xl font-semibold mt-4 mb-2 ${isLight ? 'text-gray-800' : 'text-foreground'}`} {...props} />,
    h4: ({node, ...props}: any) => <h4 className={`text-lg font-semibold mt-4 mb-2 ${isLight ? 'text-gray-800' : 'text-foreground'}`} {...props} />,
